...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, NgIf, NgFor, ReactiveFormsModule, MatFormFieldModule,
...
export class FaixaFormComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.faixaService.findModalidades().subscribe(data => {
      ...
      this.initializeForm(); // RESOLVEEEEEEEEEEEEEEEEEEEEEEEEUUUUUUUUUUUUUUUUUUUUUUUUUUUU
    });
  ...
  initializeForm(): void {
    ...
    this.formGroup = this.formBuilder.group({
      ...
      estoque: [(faixa && faixa.preco) ? faixa.preco : null],
